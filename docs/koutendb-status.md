...
| Dynamic membership / epoch migration | Foundation | Static scale-out uses a monotonic persisted placement epoch and bounded handoff. Explicit stop-the-world scale-in copies persistently drained old node directories into a fresh smaller epoch with durable checkpoints, source retention, topology fencing, version/tombstone preservation, metadata migration, and independent verification. In-place/live scale-in, mixed-epoch rolling activation, and discovery orchestration remain unsupported and fail closed |
