...
この文書は参考翻訳です。英語版 [koutendb-status.md](./koutendb-status.md) が正本であり、
この翻訳は遅れる可能性があります。
...
| 機能 | 状態 | メモ |
|---|---|---|
| Static cluster | 完了 | `koutend --id --peers` |
...
| Python | 公開済み | PyPI `koutendb` v0.3.0。Native TCP wire driver |
| Node.js / TypeScript / Bun | 公開済み | npm `koutendb` v0.1.5。Bun は実験的 |
| Rust | 公開済み | crates.io `koutendb` v0.2.0。詳細は英語正本を参照 |
| Go | 公開済み | [`koutendb-go` v0.1.0](https://github.com/puffball1567/koutendb-go); native TCP / optional embedded C ABI。導入は英語正本を参照 |
| PHP | 公開済み | Packagist `koutendb/koutendb` v0.2.0。Docker smoke あり |
...
| C++ | GitHub Release 公開済み | `koutendb-cpp` v0.2.0。Unreal 公式 asset は別候補 |
...
| 項目 | 状態 | メモ |
|---|---|---|
| Username/password auth | 完了 | koutend and driver path |
