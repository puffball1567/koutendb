...
| Web CRUD integration demos | `scripts/web_crud_demo_smoke.sh`, `examples/web/rekt-crud/`, `examples/web/prk-crud/`, `examples/web/jazzy-crud/` | Manual Docker matrix: shared React UI served through nginx, authenticated persistent KoutenDB node, REKT Node-API driver path, PRK/Jazzy public Nim API paths, shared CRUD, category-to-ring placement, tag-ranked related retrieval constrained to one ring, category relocation, and deletion. Containers and volumes are removed after the isolated smoke run. |
| Jazzy HTTP integration matrix | `scripts/jazzy_demo_smoke.sh`, `examples/web/jazzy-crud/tests/integration.py`, `.github/workflows/jazzy.yml` | Real local HTTP/TCP matrix: shared CRUD contract, malformed/oversized input, numeric ID boundaries, 32 concurrent Unicode writes/readbacks, exact list membership, DB/API restart, persistence, outage and authentication rejection, credential redaction, verified TLS, wrong hostname and untrusted CA rejection. Dedicated path-filtered CI also exercises the Docker CRUD contract; not part of core-only tests. See the [demo guide](../examples/web/jazzy-crud/README.md). |
...
| Single-node self-host operations | `scripts/container_image_smoke.sh`, `scripts/self_host_bundle_smoke.sh` | CI-covered: TLS/authenticated strong-durability bootstrap, host and runtime secret/private-key permissions, generated certificate verification and SANs, network-disabled secret staging, non-root/read-only Compose runtime, persistent write recovery after an actual `koutend` process crash, invalid-version/non-empty/symlink bootstrap refusal, consecutive-health-failure restart threshold, healthy-state reset, starting-state restraint, bounded restart-loop failure, drain/snapshot checkpoint quiet windows, duplicate-generation failure recovery, destination-staged transfer, independent restore verification, corrupt-artifact rejection, bounded credential-free operation evidence, scheduled checkpoint/export/restore transactions, exclusive scheduling, verified-generation retention, corrupt-evidence preservation, staging cleanup, systemd timer artifacts, versioned-image preflight, old/new checkpoint compatibility, successful image replacement, post-replacement health-failure rollback with write recovery, certificate/key and CA rejection, successful certificate rotation, failed TLS-health rollback, bounded capacity history, insufficient/negative-growth forecasts, content-derived plan IDs, modified/wrong/unapproved/expired/stale/repeated plan refusal, prepared-capacity thresholds, and credential-free plan state. |
