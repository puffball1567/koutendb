...
| C ABI | `examples/cabi_contract.c`, `examples/cabi_tls_contract.c`, `scripts/cabi_symbol_contract.sh`, `scripts/cabi_tls_smoke.sh`, `scripts/driver_compat.sh` | Contract-covered: complete header/shared-library symbol parity on Linux and macOS; ABI version; codec CRUD; JSON patch; count and ring pages; prepared selection prepare/query/close; nearby and anchor-relative writes; stellar attach/detach/membership/read filtering; time-orbit profile/write/range read; acknowledgement/apply/guardrail configuration; transaction visibility/commit/rollback/update; cooperative lock conflict/info/release; child-handle invalidation on DB close; strong disk-backed reopen; segment maintenance; generation checkpoints; Prometheus/OpenMetrics; NULL, malformed, range, oversized-input, invalid-codec, and stale-handle errors; Atlas; CA-verified TLS. |
...
| Web CRUD integration demos | `scripts/web_crud_demo_smoke.sh`, `examples/web/rekt-crud/`, `examples/web/prk-crud/` | Manual Docker matrix: React UI through nginx, authenticated persistent KoutenDB node, REKT Node-API driver path, PRK public Nim API path, shared CRUD, category-to-ring placement, tag-ranked related retrieval constrained to one ring, category relocation, and deletion. Images and volumes are removed after the isolated smoke run. |
