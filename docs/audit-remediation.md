...
| Durability | fsync/write failure can leave memory ahead of disk. | Done | Direct Store WAL mutations now share one failure boundary, publish in-memory changes only after persistence succeeds, poison the handle after write/flush failure, and reject later mutations. `scripts/storage_failure_matrix.sh` covers injected disk-full and short writes, failed metadata visibility, torn-tail repair, and successful retry after reopen. |
...
| Historical release stores were not exercised by the current storage engine. | Done | Immutable v0.10.1 and v0.11.0 disk-backed fixtures are generated by their tagged release code. `scripts/upgrade_fixture_matrix.sh` covers replay, legacy/current segment generations, explicit pack, exact dump comparison, JSONL migration, checkpoint restore, and offline verification. |
